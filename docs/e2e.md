...
| Floating toolbar and manipulation handles hide while dragging, then return afterward | Drag selected element | Regression deck slide 1 | Toolbar hides on drag start; resize handles and corner rotation zones disappear during drag and reappear after mouseup; no standalone rotate handle is rendered | N/A | N/A | `block-manipulation.spec.ts` |
...
| Header deck switcher loads another local manifest-backed deck | Header deck switcher menu | Regression deck plus sibling switcher deck | Switcher trigger is positioned next to the title; menu exposes deck import, current deck is marked; choosing the sibling fixture deck updates the title, stage HTML, current marker, and `/deck/manifest.json` response | History reset implied by deck reload, undo/redo not asserted | Manifest route reload asserted | `editor-chrome.spec.ts` |
| Header deck switcher imports a local manifest-backed deck | Header deck switcher menu, directory picker | Temporary manifest-backed deck folder | Import action opens a directory picker; importing a folder with `manifest.json` adds/selects the deck, updates the title and stage HTML, shows success toast, and updates `/deck/manifest.json` | History reset implied by deck reload, undo/redo not asserted | Manifest route reload asserted | `editor-chrome.spec.ts` |
...
| Image crop mode previews while dragging, commits on mouseup-exit path, and participates in undo/redo | Floating Toolbar Crop, crop handles, Enter, stage click | Regression deck slide 16 | Crop overlay and mask render; normal resize handles and corner rotation zones hide in crop mode; drag updates preview bounds without immediately writing parseable `clip-path`; stage click exits crop mode and commits rounded `clip-path`; re-enter then `Enter` exits while preserving selection; undo removes clip-path and redo restores it | Undo/redo asserted | Reload not asserted | `floating-toolbar.spec.ts` |
...
| All eight resize handles are visible for a selected element | Selection handles | Regression deck slide 1 | Four corner handles and four edge-midpoint handles are visible | N/A | N/A | `block-manipulation.spec.ts` |
| Edge resize handles constrain resizing to one axis | Edge resize handles | Regression deck slide 1 | Right-center resize changes width while preserving height and top-left origin; top-center resize changes height while preserving width and anchoring the bottom edge | Undo/redo not asserted | Not asserted | `block-manipulation.spec.ts` |
| Corner rotation zones trigger rotation without a standalone handle | Corner rotation zones | Regression deck slide 1 | A corner-adjacent rotation zone is visible, the old standalone rotate handle is absent, and dragging the zone writes a transform without using resize handles | Undo/redo not asserted | Not asserted | `block-manipulation.spec.ts` |
...
| Presenter View HTML export opens directly in standalone presenter mode | Export menu, downloaded file | Regression deck | Downloaded HTML opens with presenter root visible; toolbar shows page count; next-slide button and slide-frame click both advance slides | N/A | Standalone reopen asserted | `presenter-mode.spec.ts` |
